...
import Svg, { Path, Text as SvgText, G, Circle, Defs, LinearGradient, Stop, DropShadow } from "react-native-svg";
...
const Roulette = () => {
  ...
  const spinWheel = async () => {
    ...
    try {
      ...
      const game = await rouletteUseCase.playGame(user?.id || 'test-user', SPIN_COST);
      
      // Para fase de teste, apenas simular
      if (user) {
        const updatedUser = await userUseCase.updateUserPoints(user.id, Math.max(0, user.points - SPIN_COST + game.result.pointsWon));
        setUser(updatedUser);
      }
  ...
  const pointsInfo = user ? `Você possui ${user.points} pontos. Modo teste: giros liberados!` : "Modo teste: giros liberados!";
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.wheelWrapper}>
          ...
          >
            <Svg width={WHEEL_SIZE} height={WHEEL_SIZE}>
              <Defs>
                <LinearGradient id="wheelShadow" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              {/* Pointer */}
              <Path
                d={`M ${CENTER_X} ${CENTER_Y - RADIUS - 10} L ${CENTER_X - 8} ${CENTER_Y - RADIUS + 8} L ${CENTER_X + 8} ${CENTER_Y - RADIUS + 8} Z`}
                fill="#333333"
                stroke="#000000"
                strokeWidth="1"
              />
      ...
      <View style={styles.instructionsContainer}>
        <Text style={styles.instructionsText}>
          {spinning ? '🎰 Girando...' : '👆 Toque no centro da roleta para girar'}
        </Text>
        
        {/* Botão de backup para girar */}
        <TouchableOpacity
          style={[styles.backupSpinButton, spinButtonDisabled && styles.disabledBackupButton]}
          onPress={spinWheel}
          disabled={spinButtonDisabled}
        >
