...
const HistoryList: React.FC = () => {
  ...
  const loadEntries = useCallback(async () => {
    try {
      ...
      if (stored) {
        type StoredEntry = Omit<HistoryEntry, "data"> & { data: string };
  ...
  const orderedEntries = useMemo(
    () => [...entries].sort((a, b) => b.data.getTime() - a.data.getTime()),
    [entries]
  );
      ...
      <View style={styles.container}>
        ...
        {orderedEntries.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>Você ainda não possui lançamentos.</Text>
          </View>
        ) : (
          <ScrollView
            style={styles.list}
              ...
              <View key={entry.id} style={styles.card}>
                ...
                <View style={styles.cardRight}>
                  ...
                  <Text style={styles.cardAmount}>
                    {`-R$ ${Number(entry.valor).toLocaleString("pt-BR", {
